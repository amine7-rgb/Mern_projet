...
module.exports = function(app) {
  ...
  app.post(
    ...
    [
      ...
      verifySignUp.checkDuplicateCinOrTEL
    ],
    controller.signup
  );
  app.post(
    "/api/auth/addadmin",fileUpload,
    [
      verifySignUp.checkDuplicateUsernameOrEmail,
      verifySignUp.checkRolesExisted , 
      verifySignUp.checkDuplicateCinOrTEL
  ...
  app.post("/api/auth/send", controller.sendEmail);
  
  app.post("/facture",controller.facture)
  app.post("/api/auth/devis", controller.createQuote);
...
app.post("/api/auth/reset", controller.resetPassword);
 app.get("/verify-email/:id", controller.verifyEmail)
  ...
  app.get("/Countusers", controller.countAllUsers);
  app.get("/Countquote", controller.countAllQuote);
  app.get("/CountFacture", controller.countAllFacture);
  app.get("/CountTotalP", controller.countAllProducts);
  ...
  app.get('/api/auth/getUsername/:username', controller.getUserByUsername);

  app.get('/api/auth/bannedUser/:id', controller.bannedUser);
  app.get('/api/auth/unbannedUser/:id', controller.unbanUser);
  /////////////////FACEID///////////////////
  app.post('/api/sensorsKmeans', controller.kmeans)
  ...
  // app.put('/api/auth/update2/:id',
  
  
  // controller.updateUserById2,
  // );
};
